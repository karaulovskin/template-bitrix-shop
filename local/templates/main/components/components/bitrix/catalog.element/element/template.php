...
if($arResult["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > $price_add && $price_add>0){
	$proc = round(($arResult["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"]-$price_add) / $arResult["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] * 100);
}
else $proc = false;	
?>
            ...
            <div class="slider-catalog__container swiper-container">
                ...
                <div class="slider-card__content">
                    <div class="icon-info-list">
                        ...
                        <?endif;?>
                        <?if($proc > 0 && $arResult["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > 0):?>
	                        <div class="icon-info-list__item">
                    ...
                    <div class="card-item__bottom">
                    	<div class="card-item__article"><?if($arResult['PROPERTIES']['CML2_ARTICLE']['VALUE']):?>Арт: <?=$arResult['PROPERTIES']['CML2_ARTICLE']['VALUE'];?><?endif;?></div>
                    	<div class="card-item__buy">
	                        ...
	                        <div class="card-item__price">
	                        	<div class="price price--big">
	                                <?if($price):?>
	                                	<?if($arResult["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > $price && $arResult["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"]>"0"):?>
		                                	<div class="price__value catalog-item-price__value--sale"><?=$arResult["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"];?> р</div>
	                            ...
	                            <div class="card-item__note">
	                            	<?if(strtotime($arResult['PROPERTIES']['SROK_DEYSTVIYA_SKIDKI']['VALUE'])>=strtotime(date('d.m.Y')) && $arResult['PROPERTIES']['SROK_DEYSTVIYA_SKIDKI']['VALUE']):?><div class="card-item-note">Скидка действует до <?=$arResult['PROPERTIES']['SROK_DEYSTVIYA_SKIDKI']['VALUE'];?></div><?endif;?>
	                            	<?if($arResult["CATALOG_QUANTITY"]==0 || $price==0):?>
		        ...
		        <div class="card-kit">
				    ...
				    <div class="card-kit__body">
				    	<div class="catalog-list">
							...
							if($elem["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > $pricemadd){
								$procm = round(($elem["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"]-$pricemadd) / $elem["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] * 100);
							}
							else $procm = false;	
				    		?>
					            <div class="catalog-list__item catalog-list__item--complete">
					                <div class="catalog-item">
					                    ...
					                    <div class="catalog-item__bottom">
					                        <div class="catalog-item-price">
				                                <?if($elem["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > $pricem && $elem["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"]>"0"):?>
					                                <div class="price__value catalog-item-price__value--sale"><?=$elem["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"];?> р</div>
        ...
        <div class="tabs" data-tabs>
            ...
            <div class="tabs-content" data-tabs-content>
                ...
                <?if($arResult["PROPERTIES"]["video"]["VALUE"]):?>
	                <div class="tabs-content__item" data-tabs-content-item>
	                    <div class="tabs-content__head">
	                        <div class="h3 title-border">Видео</div>
	                    </div>
               ...
               <?$res= CCatalogStore::GetList(array(), array('PRODUCT_ID' =>$arResult["ID"]), false, false, array('PRODUCT_AMOUNT', 'XML_ID')); 
                ...
                <div class="tabs-content__item" data-tabs-content-item>
                    <div class="tabs-content__head">
                        <div class="h3 title-border">Наличие</div>
                    </div>
...
<section class="section-catalog">
    <div class="container">
    	<?if($arResult["PROPERTIES"]["SOPUTSTVUYUSHCHIE_TOVARY"]["VALUE"]):?>
    	    	<?$soputst = explode("#",$arResult["PROPERTIES"]["SOPUTSTVUYUSHCHIE_TOVARY"]["VALUE"]);?>
    		<?foreach($soputst as $isec=>$goodslist){
	    		$catlist = CIBlockElement::GetList(Array(),Array("XML_ID"=>$goodslist,"ACTIVE"=>"Y"),false,false,array("IBLOCK_SECTION_ID","ID"))->GetNext();
	    		$res = CIBlockSection::GetByID($catlist["IBLOCK_SECTION_ID"])->GetNext();
	    		if($res["IBLOCK_SECTION_ID"]){
	    			$sectid = $res["IBLOCK_SECTION_ID"];
	    		}
	    		else{
	    			$sectid = $res["ID"];
	    		}
	    			$sectionlist[] = $sectid;
	    		if($isec=="0"){$onesect =$sectid;}
	    			$elementlist[$sectid][] = $catlist["ID"];
	    	}
	            ...
	            <div id="sliderHit" class="slider-catalog swiper-ajax">
	                ...
	                <div class="slider-catalog__container swiper-container">
	                    <div class="slider-catalog__wrapper swiper-wrapper">
							...
							if($compgood["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > $price){
								$proc = round(($compgood["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"]-$price) / $compgood["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] * 100);
							}
							else $proc = false;	
	                    	?>
		                        <div class="slider-catalog__slide swiper-slide">
		                            <div class="catalog-item">
		                                <div class="icon-info-list">
				                            ...
				                            <?endif;?>                           
			                                <?if($proc > 0 && $compgood["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > 0):?>
				                                <div class="icon-info-list__item">
				                                    <div class="icon-info icon-info--discount">-<?=$proc;?>%</div>
				                                </div>	                                
			                                <?endif;?>
		                                ...
		                                <div class="catalog-item__bottom">
		                                    <div class="catalog-item-price">
			                                	<?if($compgood["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"] > $price && $compgood["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"]>"0"):?>
				                                    <div class="catalog-item-price__value catalog-item-price__value--sale"><?=$compgood["PROPERTIES"]["PERECHERKNUTAYA_TSENA"]["VALUE"];?> р</div>
				                                    <div class="catalog-item-price__discount"><?=$price;?> р</div>	                                	
			                                	<?elseif($price):?>
			                                    	<div class="catalog-item-price__value"><?=$price;?> р</div>
			                                    <?endif;?>
		                                    </div>
		                                   <?if($compgood["PROPERTIES"]["quan"]["VALUE"] && $price >0):?>
		                                                <a class="catalog-item__basket to_cart" data-rel="<?=$compgood["ID"];?>">
		                                                    <svg class="i-icon">
		                                                        <use xlink:href="#icon-basket-catalog"></use>
		                                                    </svg>
		                                                </a>
	                                            <?else:?>
	                                	            <a href="#coming" class="catalog-item__coming" data-modal-open data-rel="<?=$compgood["ID"];?>">Сообщить о поступлении</a>
	                                            <?endif;?>
		                                </div>
		                            </div>
		                        </div>

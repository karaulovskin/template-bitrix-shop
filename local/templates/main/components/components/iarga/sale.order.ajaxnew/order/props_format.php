...
if (!function_exists("anPrintPropsForm"))
{
	function anPrintPropsForm($arSource = array(), $locationTemplate = ".default")
	{
		if (!empty($arSource))
		{
				if($arSource[0]["ID"]=="18" || $arSource[0]["ID"]=="19"){
				?>	
						...
						<div class="form-col">
							...
							foreach ($arSource as $arProperties)
							{
								?>
									...
									{?>
										<label class="label" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>" <?if($arProperties["ID"]=="7" || $arProperties["ID"]=="13" || $arProperties["ID"]=="26" || $arProperties["ID"]=="27"):?>style="display:none;"<?endif;?>>
											<input type="text" class="input<?if($arProperties["REQUIED_FORMATED"]=="Y"):?> input--required<?endif;?>" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"  placeholder="" <?if($arProperties["REQUIED_FORMATED"]=="Y"):?> data-input-required="<?=$arProperties["CODE"];?>"<?endif;?> />
		                                    <span class="placeholder">
		                                        <span><?=$arProperties["NAME"];?></span>
		                                        <?if($arProperties["REQUIED_FORMATED"]=="Y"):?><span class="required">*</span><?endif;?>
		                                    </span>
										</label>
										<?
									}
                	...
                	<div class="form-row">
						...
						foreach ($arSource as $arProperties)
						{
								...
								{ if($arProperties["ID"]!="15"){$s++;}
									$in=$s%2;?>
									<div class="form-col <?if($arProperties["ID"]=="15" || $arProperties["ID"]=="16"):?>form-col--3<?else:?>form-col--6<?endif;?>" <?if($arProperties["ID"]=="24" || $arProperties["ID"]=="25"):?> style="display:none;"<?endif;?>>
										<label class="label" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
											<input type="text" class="input<?if($arProperties["REQUIED_FORMATED"]=="Y"):?> input--required<?endif;?>" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$User[$arProperties["CODE"]]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="" <?if($arProperties["REQUIED_FORMATED"]=="Y"):?> data-input-required="<?=$arProperties["CODE"];?>"<?endif;?> />
		                                    <span class="placeholder">
		                                        <span><?=$arProperties["NAME"];?></span>
		                                        <?if($arProperties["REQUIED_FORMATED"]=="Y"):?><span class="required">*</span><?endif;?>
		                                    </span>
	...
	function chekPrintPropsForm($arSource = array())
	{
		foreach ($arSource as $arProperties)
		{
			...
			{?>
				...
				<input type="checkbox" name="<?=$arProperties["FIELD_NAME"]?>" class="checkbox__input checkbox--required" id="<?=$arProperties["FIELD_NAME"]?>" value="">
			<?
			}	
		}	
	}
